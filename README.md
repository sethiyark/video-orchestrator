...
- **images:** one illustration per chapter and per scene (background plates and `ImagePan` subjects). By default every profile routes `assets` to **manual**: the dashboard lists the prompts, you generate the images in your own Claude Pro or Gemini Pro chat and upload them. Switch `routes.assets` to `images` for local generation: the CUDA profile uses FLUX.1-schnell GGUF (Q4_K_S transformer + GGUF T5 encoder, CPU offload, 4 steps); the Mac profile uses SDXL-Turbo on Metal (4 steps, guidance 0), 24 prompts per model load, cached across videos by prompt. Images stay disabled in the default CPU `models.yaml`.
...
Storyboards are planned as chapters (an LLM pass over the script, falling back to outline sections) and accept a closed JSON schema of fourteen scene types: `ChapterTitle`, `DefinitionCard`, `AnimatedFlowDiagram`, `BulletReveal`, `CodeBlock`, `Terminal`, `Comparison`, `StatCounter`, `Timeline`, `Callout` (a quote that must appear in a verified claim), `IconGrid`, `ImagePan`, `SeriesAsset` (an existing series image or logo, by id) and a closing `Outro`. Every scene carries an image prompt. They cannot contain arbitrary renderer code. Kokoro (or Qwen3-TTS) produces a concatenated WAV with chunk boundaries; alignment produces segment and word timestamps plus a fidelity score against the script, and stops the job for review when the audio does not match the narration. The renderer retimes scenes using alignment and the actual WAV duration.
